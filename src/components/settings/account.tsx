...
export default function Account() {
  ...
  return (
    <div className='flex flex-col items-center gap-2'>
      {inputs.map((input, key) => <Input key={key} {...defaultInputProps} {...input} className={cn(defaultInputProps.className, input.className)} />)}
      {buttons.map((button, key) => <AnimatedIconButton key={key} {...defaultButtonProps} {...button} endContent={<div className='text-lg' >{button.endContent}</div>} className={cn(defaultButtonProps.className, button.className)} />)}
    </div>
  );
}
